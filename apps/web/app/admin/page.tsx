...
type Worker={worker_name:string;last_succeeded_at:string|null;last_failed_at:string|null;last_duration_ms:number|null;last_error:string|null;updated_at:string}
type LegalAcceptance={id:string;user_id:string;document_version:string;accepted_terms:boolean;accepted_privacy:boolean;accepted_risk:boolean;age_confirmed:boolean;evidence_hash:string;accepted_at:string}
...
export default async function AdminPage(){
 ...
 const [orders,profiles,payouts,accounts,risks,audit,tickets,workers,acceptances]=await Promise.all([
  ...
  isAdmin?getOwnData<Worker[]>('worker_heartbeats?select=worker_name,last_succeeded_at,last_failed_at,last_duration_ms,last_error,updated_at&order=worker_name',auth.accessToken):Promise.resolve([]),
  isAdmin?getOwnData<LegalAcceptance[]>('legal_acceptances?select=id,user_id,document_version,accepted_terms,accepted_privacy,accepted_risk,age_confirmed,evidence_hash,accepted_at&order=accepted_at.desc&limit=200',auth.accessToken):Promise.resolve([])
 ])
 ...
 return <main className={styles.page}>
  ...
  <section><Title label="Payout operations" count={payouts.length}/><div className={styles.grid}>{payouts.map(p=>{const network=p.method.replace('USDT_','');return <article key={p.id} className={styles.card}><Head status={p.status} date={p.requested_at}/><h3>{Number(p.amount).toFixed(2)} {p.currency}</h3><dl><Info label="Trader" value={names.get(p.user_id)||p.user_id.slice(0,8)}/><Info label="Network" value={p.method}/></dl><label>Destination</label><code>{p.destination_ref||'—'}</code>{p.destination_ref&&<a target="_blank" rel="noreferrer" href={addressExplorer(network,p.destination_ref)}>Inspect destination ↗</a>}{p.provider_payout_id&&<><label>Payout transaction</label><code>{p.provider_payout_id}</code><a target="_blank" rel="noreferrer" href={explorer(network,p.provider_payout_id)}>Verify payout transaction ↗</a></>}<PayoutActions id={p.id} status={p.status}/></article>})}</div></section>
  ...
  <section><Title label="Automation health" count={workers.length}/><div className={styles.table}>{workers.map(w=>{const stale=!w.last_succeeded_at||Date.now()-new Date(w.last_succeeded_at).getTime()>15*60*1000;return <article key={w.worker_name}><div><b>{w.worker_name}</b><small>{w.last_error||'No reported error'}</small></div><strong>{stale?'STALE':'HEALTHY'}</strong><span>{w.last_duration_ms??0} ms</span><time>{w.last_succeeded_at?new Date(w.last_succeeded_at).toLocaleString('tr-TR'):'Never succeeded'}</time></article>})}</div></section>
  <section><Title label="Legal acceptance evidence" count={acceptances.length}/><div className={styles.table}>{acceptances.map(a=><article key={a.id}><div><b>{names.get(a.user_id)||a.user_id.slice(0,8)}</b><small>Version {a.document_version} · evidence {a.evidence_hash.slice(0,16)}…</small></div><strong>{a.accepted_terms&&a.accepted_privacy&&a.accepted_risk&&a.age_confirmed?'COMPLETE':'INCOMPLETE'}</strong><span>Terms · Privacy · Risk · 18+</span><time>{new Date(a.accepted_at).toLocaleString('tr-TR')}</time></article>)}</div></section>
...
function addressExplorer(n:string,a:string){if(n==='BEP20')return'https://bscscan.com/address/'+a;if(n==='ERC20')return'https://etherscan.io/address/'+a;if(n==='TRC20')return'https://tronscan.org/#/address/'+a;return'https://tonviewer.com/'+a}
