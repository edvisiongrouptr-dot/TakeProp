...
export default function ResetPassword(){
 ...
 return <main className="authPage"><section className="authIntro"><BrandLogo priority/><div><span>ACCOUNT SECURITY</span><h1>Choose a new password.</h1><p>Use at least 12 characters with upper and lowercase letters, a number and a symbol.</p></div></section><section className="authPanel"><div className="authBox"><span>SECURE RECOVERY</span><h2>Reset password.</h2>{done?<><div className="authMessage">{message}</div><Link className="btn authSubmit" href="/auth">Sign in →</Link></>:<form onSubmit={submit}><label>NEW PASSWORD<input name="password" type="password" autoComplete="new-password" minLength={12} maxLength={128} required/></label><label>CONFIRM PASSWORD<input name="confirm" type="password" autoComplete="new-password" minLength={12} maxLength={128} required/></label>{message&&<div className="authMessage">{message}</div>}<button className="btn authSubmit" disabled={loading||!token}>{loading?'Please wait…':'Update password →'}</button></form>}</div></section></main>
}
